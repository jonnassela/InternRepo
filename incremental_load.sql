...
-- fixed issue if last_load_time is null and has no value inserted 
--commit 2 
GO
CREATE OR ALTER PROCEDURE incremental_load_customers
AS
BEGIN
    ...
    BEGIN TRY
        ...
        -- Step 1: Get last load time in this case set a default last load time so it doesnt throw an null error
        SELECT @last_load = ISNULL(last_load_time, '1900-01-01')
        FROM config.load_config;
        -- Step 2: Update changed rows
        UPDATE s
        SET 
